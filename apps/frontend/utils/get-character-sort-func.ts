...
export const getCharacterSortFunc = (prefixFunc?: SortValueFunction, viewSortBy?: string[]) => {
    ...
    return (char: Character) => {
        ...
        const index = settingsState.value.characters.pinnedCharacters?.indexOf(char.id) ?? -1;
        ...
        for (const thing of sortBy) {
            if (thing === 'account') {
                ...
                const enabled = settingsState.value.accounts?.[char.accountId]?.enabled ?? true;
                ...
                const hasFlag =
                    ((settingsState.value.characters.flags?.[char.id] || 0) & (1 << tagId)) > 0;
