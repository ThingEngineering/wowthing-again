...
function buildAppearanceCache(stores: LazyStores) {
    ...
    for (const item of Object.values(stores.itemData.items)) {
        if (!item.appearances || byExpansion[item.expansion] === undefined) { continue }

        const itemKey = (((keyCache[item.expansion] ||= {})[item.classId] ||= {})[item.subclassId] ||= {})[item.inventoryType] ||= [item.expansion, item.classId, item.subclassId, item.inventoryType].join('|')
    ...
    for (const [key, appearanceMap] of Object.entries(byAppearanceId)) {
        const [expansion, classId, subclassId, inventoryType] = key.split('|').map(n => parseInt(n))
        ...
        const sortedData = sortBy(
            Object.entries(appearanceMap),
            ([, arrays]) => [
                leftPad(999999 - Math.min(...arrays.map((array) => array[0])), 6, '0'),
                leftPad(99 - Math.min(...arrays.map((array) => modifierSortOrder[array[2]] || 0)), 2, '0'),
            ].join('|')
        )
        ...
        for (const [appearanceId, appearanceArrays] of sortedData) {
            nameMap.appearances.push(new AppearanceDataAppearance(
    ...
    for (const [expansion, sets] of sortBy(Object.entries(appearanceCache), ([exp,]) => 100 - parseInt(exp))) {
        for (const set of sets) {
            let typeName: string
