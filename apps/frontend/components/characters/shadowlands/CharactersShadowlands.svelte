<script lang="ts">
    ...
    $: {
        ...
        if (covenantSlugMap[params.slug4] === undefined && params.slug4 !== 'cypher-research') {
            replace(
                `${baseUrl}/${covenantMap[character.shadowlands?.covenantId || covenantOrder[0]].slug}`
            );
        }
    }
</script>
...
<nav class="subnav">
    {#each covenantOrder as covenantId}
        ...
        <a
            href="#/characters/{params.slug1}/{params.slug2}/{params.slug3}/{covenantMap[covenantId]
                .slug}"
            use:active
        >
            <WowthingImage name={covenantMap[covenantId].icon} size={20} border={0} />
            {covenantMap[covenantId].name}
            <pre class={getPercentClass((renown / Constants.maxRenown) * 100)}>{@html leftPad(
                    renown,
                    2
                )}</pre>
        </a>
    ...
    <a
        ...
        use:active
    >
        <WowthingImage name="currency/1979" size={20} border={0} />
        Cypher Research
    </a>
...
{#if params.slug4}
    {#if params.slug4 === 'cypher-research'}
        <GarrisonTree tree={garrisonTrees.cypherResearch} {character} />
    {:else}
        <Covenant covenantId={covenantSlugMap[params.slug4].id} {character} />
    {/if}
{/if}
