<script lang="ts">
    ...
    $: {
        ...
        for (let quality = 0; quality < qualities.length; quality++) {
            parts.push(`<div style="display:flex;gap:1.5rem;justify-content:space-between;"><span class="quality${quality}">${ItemQuality[quality]}</span><span>${qualities[quality]}</span></div>`);
        }
        tooltip = parts.join('');
    }
</script>
