        ...
        for (const maps of wowthingData.manual.zoneMaps.sets) {
            ...
            for (const map of maps.slice(1)) {
                ...
                const mapTypeCounts: Record<number, UserCount> = (typeCounts[mapKey] =
                    Object.fromEntries(
                        Object.keys(LookupType).map((key) => [key, new UserCount()])
                    ));
                ...
                for (const vendorId of wowthingData.manual.shared.vendorsByMap[map.mapName] || []) {
                    farms.push(
                        ...wowthingData.manual.shared.vendors[vendorId].asFarms(map.mapName)
                    );
                }
                ...
                for (const farm of farms) {
                    ...
                    for (const drop of farm.drops || []) {
                        ...
                        dropStatus.skip =
                            ...
                            (lookupType === LookupType.Mount && !zoneMapStateValue.trackMounts) ||
                            (lookupType === LookupType.Pet && !zoneMapStateValue.trackPets) ||
                            (lookupType === LookupType.Toy && !zoneMapStateValue.trackToys) ||
                        ...
                        if (!dropStatus.skip) {
                            ...
                            if (mapSeen[drop.type][seenId] === undefined || special) {
                                ...
                                mapTypeCounts[lookupType].total += totalInc;
                            }
                            ...
                            if (!dropStatus.need) {
                                ...
                                if (mapSeen[drop.type][seenId] === undefined || special) {
                                    ...
                                    mapTypeCounts[lookupType].have += haveInc;
                                }
                            }
                        ...
                        if (dropStatus.need && !dropStatus.skip) {
                            ...
                            if (drop.limit?.length > 0) {
                                switch (drop.limit[0]) {
                                    ...
                                    case 'profession': {
                                        const professionId =
                                            wowthingData.static.professionBySlug.get(
                                                drop.limit[1]
                                            )?.id;
                                        ...
                                        if (drop.limit.length === 4) {
                                            if (drop.limit[2].match(/^\d+$/)) {
                                                ...
                                                const expansion =
                                                    expansionShortNameMap[drop.limit[2]];
                                                subProfessionId =
                                                    wowthingData.static.professionById.get(
                                                        professionId
                            ...
                            for (const character of dropCharacters) {
                                ...
                                if (farm.type === FarmType.Quest) {
                                    if (
                                        farm.questIds.every(
                                            (q) => !charQuests?.hasQuestById?.has(q)
                                        )
                                    ) {
                                    ...
                                    const professionInfo =
                                        wowthingData.static.professionAbilityByItemId.get(drop.id);
                                    if (
                                        !character.knowsProfessionAbility(professionInfo.abilityId)
                                    ) {
                                        dropStatus.characterIds.push(character.id);
                                    } else {
                                        // dropStatus.completedCharacterIds.push(character.id)
...
const lazyZoneMapsProcessor = new LazyZoneMapsProcessor();

export function doZoneMaps(): LazyZoneMaps {
    console.time('LazyState.doZoneMaps');

    const result = lazyZoneMapsProcessor.process();
