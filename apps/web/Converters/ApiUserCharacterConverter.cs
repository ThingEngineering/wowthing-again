...
public class ApiUserCharacterConverter : JsonConverter<ApiUserCharacter>
{
    ...
    public override void Write(Utf8JsonWriter writer, ApiUserCharacter character, JsonSerializerOptions options)
    {
        ...
        JsonSerializer.Serialize(writer, character.Lockouts, options); // 30
        JsonSerializer.Serialize(writer, character.MythicPlus, options); // 31
        JsonSerializer.Serialize(writer, character.MythicPlusAddon, options); // 32
        JsonSerializer.Serialize(writer, character.MythicPlusSeasons, options); // 33
        JsonSerializer.Serialize(writer, character.Paragons, options); // 34
        JsonSerializer.Serialize(writer, character.PatronOrders, options); // 35
        JsonSerializer.Serialize(writer, character.Professions, options); // 36
        JsonSerializer.Serialize(writer, character.ProfessionCooldowns, options); // 37
        JsonSerializer.Serialize(writer, character.ProfessionSpecializations, options); // 38
        JsonSerializer.Serialize(writer, character.ProfessionTraits, options); // 39
        JsonSerializer.Serialize(writer, character.RaiderIo, options); // 40
        JsonSerializer.Serialize(writer, character.Reputations, options); // 41
        JsonSerializer.Serialize(writer, character.Shadowlands, options); // 42
        JsonSerializer.Serialize(writer, character.Weekly, options); // 43

        JsonSerializer.Serialize(writer, character.RawCurrencies, options); // 44
        JsonSerializer.Serialize(writer, character.RawItems, options); // 45
        JsonSerializer.Serialize(writer, character.RawMythicPlusWeeks, options); // 46
        JsonSerializer.Serialize(writer, character.RawSpecializations, options); // 47
        JsonSerializer.Serialize(writer, character.RawStatistics, options); // 48
